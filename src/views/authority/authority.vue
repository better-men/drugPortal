...
<section>
    ...
    <el-table :data="userList.slice((currentPage-1)*pagesize,currentPage*pagesize)" highlight-current-row v-loading="listLoading" style="width: 100%;">
        ...
        <!-- <el-table-column prop="disable" label="是否禁用" width="100" :filters="[{ text: '是', value: '是' }, { text: '否', value: '否' }]" :filter-method="filterDisable" filter-placement="bottom-end">
            <template slot-scope="scope">
                <el-tag :type="scope.row.tag === '否' ? 'primary' : 'success'" disable-transitions>{{scope.row.disable}}</el-tag>
            </template>
        </el-table-column> -->
    ...
    <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage" :page-sizes="[10, 20, 50, 100]" :page-size="pagesize" layout="total, sizes, prev, pager, next, jumper" :total="userList.length">
    </el-pagination>
</section>
</template>
